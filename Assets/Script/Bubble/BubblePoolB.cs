...
public class BubblePoolB : ObjPool<BubblePoolB, Bubble>
{
    ...
    protected override Bubble IniObj(Bubble obj) //每次调用前的初始化 
    {
        ...
        obj.bubble.GetComponent<SpriteRenderer>().sprite = BubblePoolA.Instance.B;
        obj.ani.gameObject.GetComponent<SpriteRenderer>().color =  new Color(23 / 255f, 186 / 255f, 229 / 255f);
    ...
    private void Update() // 飞起来
    {
        foreach (Bubble a in Bubbles.Values)
        {
            ...
            {
                if (!a.isA)
                    a.bubble.transform.position +=
                        new Vector3(_distance * a.dis, a.speed * Time.deltaTime * speedChange, 0);
                else
                    a.bubble.transform.transform.position += new Vector3(_distance * a.dis,
                        a.speed * Time.deltaTime * BubblePoolA.Instance.speedChange, 0); //其实是 Bu
            }
        }
    }
    ...
    public void changeBubble(int num)
    {
        ...
        if (count != 0)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                Bubbles.ElementAt(index[i]).Value.ani.gameObject.GetComponent<SpriteRenderer>().color =
                    new Color(224 / 255f, 0/ 255f, 0 / 255f);
            }
        }
    }
